...
public class Strategy4 {
    ...
    public static int iter_n;

    public static void resetScanner() throws IOException{
        if (iterScanner != null)
        {
            iterScanner.close();
        }
        iterScanner = new Scanner(new File("result_80000.txt"));
        iter1 = iterScanner.nextLine();
        iter_n = 0;
    }
    ...
    public static ArrayList<Float> getSolValue(int index) throws IOException{
        ArrayList<Float> list_val = new ArrayList<>();
        ...
        for (int i = 0; i < index; i++) {
            ...
            String final_r2 = scanner.nextLine();
            String[] final_r = final_r2.split("\t");
            int n = Integer.parseInt(final_r[4]); // n : 각 문제에서의 결정 변수들의 갯수
            // 결정변수와 그에 해당하는 값들을 저장
            if (Objects.equals(final_r[0], "optimal")) {
                //status	primal objective	gap	   solve_time	number_of_var	Vars	    value
                for (int j = 0; j < n; j++) {
                    list_val.add(Float.parseFloat(final_r[5 + j + n]));
                }
            }
        scanner.close();
        return list_val;
    ...
    public static ArrayList<Float> getSolValue_iter(int index) throws IOException{
        ArrayList<Float> list_val_iter = new ArrayList<>();
        if(iterScanner == null || index < iter_n || !iterScanner.hasNextLine()){
            resetScanner();
        }
        while(index > iter_n){
            iterScanner.nextLine();
            iter_n++;
        }
        String iter_r2 = iterScanner.nextLine();
        iter_n++;
        String[] iter_r = iter_r2.split("\t");
        int n = Integer.parseInt(iter_r[4]); // n : 각 문제에서의 결정 변수들의 갯수
        // 결정변수와 그에 해당하는 값들을 저장
        
        if (Objects.equals(iter_r[0], "optimal")) {
            //status	primal objective	gap	   solve_time	number_of_var	Vars	    value
            for (int j = 0; j < n; j++) {
                list_val_iter.add(Float.parseFloat(iter_r[5 + j + n]));
            }
        }
        return list_val_iter;
    }
  ...
  // start read
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (Objects.equals(r[0], "optimal")) {
                //status	primal objective	gap	   solve_time	number_of_var	Vars
                for (int j = 0; j < n; j++) {
            ...
            try {
                ...
                if (main_choice == 1) {
                    ...
                    if (sol[sol_n-1].status.equals("optimal")) {
                        ...
                        fw2.write(getSolValue(sol_n-1).toString());
                        fw2.close();
                    ...
                    for (int i = 0; i < num; i++) {
                        if (sol[i].sol_key.contains(key) && sol[i].status.equals("optimal")) {
                            ...
                            if (main_choice == 7) {
                                String str = "x" + key + "가 포함된 문제 중 풀린 문제 번호: " + (i + 1) + "의 최적해 값들 -> " + getSolValue_iter(i);
                                fw.write(str+"\n");
                                //System.out.println(str);
                    ...
                    for (int i = 0; i < num; i++) {
                        if (sol[i].sol_key.contains(nn) && sol[i].status.equals("optimal")) {
                            ...
                            float solvsrange = getSolValue_iter(i).get(index);

                            if (solvsrange > range && sign.equals(">")) {
                                s.add(i+1);
                                //System.out.print(i+1 + " ");
                                cnt++;
                            }
                            else if (solvsrange < range && sign.equals("<")) {
                                //System.out.print(i+1 + " ");
                                s.add(i+1);
                                cnt++;
                            }
